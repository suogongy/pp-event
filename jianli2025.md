...
- 作为技术**负责人**，支持**税务**相关系统迭代，实现公司内进项发票池业务系统，实现平台主播税务信息报送系统（2024-2025）
- 作为技术**负责人**，从0到1实现xi-event**异步事件框架**，从0到1搭建会员**精细化运营**系统（2022）
- 作为技术**负责人**，从0到1搭建喜马拉雅**家庭会员**业务系统（2021-2023）
- 作为技术**负责人**，重构喜马拉雅商品服务逻辑，重写兑换码系统（2020-2020）
- 作为**核心研发**与技术**负责人**，从0到1搭建喜马拉雅**内容付费**系统，实现初版的商品、交易、支付、履约等基建能力，支持专栏、有声书、问答、会员等业务（2016-2019）
...
- **开发语言与框架**：Java、Spring、MyBatis等
- **数据存储**：MySQL、Redis等
- **系统架构**：分布式系统架构与设计，常用分布式中间件，微服务架构，大数据量、高并发、高性能、高可用系统
- **系统设计**：熟悉常用设计模式，有较丰富的系统分析、设计和重构经验
- **新兴技术**：对LLM、AI Agent、Web3和K8s等有一定了解
...
**主要工作：** 基本独立负责相关业务和技术调研、系统搭建和自测等，通过插件式开发集成不同的LLM、文生图模型和TTS模型等，通过LangGraph实现多智能体架构，在实现环节主要作为一个引导者使用cursor实现  
**结果说明：** 系统未完成上线，已完成调研和设计，前端页面系统搭建完成，后端基础组件能力基本实现，但提示词优化、智能体协作、多插件配置、视频合成以及前后端联调工作未完成
...
### ⚡ Xi-Event异步事件框架(2022.03-2022.06)

**背景介绍：** 业务中有很多跨系统的交互希望进行可靠的异步化处理，传统的分布式事务过重，但公司缺乏相关的轻量级组件，于是决定自研  
**任务描述：** 独立设计一套异步事件框架，要求易于接入业务系统，足够可靠，事件方便监控和管理，多次重试后依然失败则自动告警  
**主要工作：** 独立负责相关技术调研、设计、开发、自测和宣讲。最终基于xxl-job，disruptor实现，支持默认配置和自定义配置  
**结果说明：** 因后续新业务系统不多，最终在会员技术团队内部的2套新系统中引入该事件框架。组件运行稳定、符合预期
