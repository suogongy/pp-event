...
public interface PPEventMapper {
    ...
    @Select("SELECT * FROM PP_EVENT WHERE id = #{id}")
    ...
    @Select({"<script>" +
            "SELECT * FROM PP_EVENT WHERE id in" +
    ...
    @Select("SELECT * FROM PP_EVENT WHERE status = #{status} AND id > #{preId} AND create_time < #{createTimeThreshold} ORDER BY ID ASC LIMIT #{pageSize}")
    @ResultMap(value = "ppEventMap")
    ...
    @Select("SELECT * FROM PP_EVENT WHERE id > 0 ORDER BY ID DESC LIMIT #{offset},#{pageSize}")
    @ResultMap(value = "ppEventMap")
    List<PPEvent> findWithPaging(@Param("offset") long offset, @Param("pageSize") int pageSize);

    @Select("SELECT * FROM PP_EVENT WHERE status = 3 AND id > #{preId} ORDER BY ID ASC LIMIT #{pageSize}")
    List<PPEvent> findFailedEventsByPreIdWithPaging(@Param("preId") long preId, @Param("pageSize") int pageSize);

    @Insert({"<script>" +
            "insert into PP_EVENT(event_no,status, retried_count,method_invocation_content,create_time,update_time,version) values " +
    ...
    @Update({"update PP_EVENT set status = #{status},retried_count=#{retriedCount},update_time = now(),version=#{version} where id=#{id} and version = #{version}-1"})
    int update(PPEvent ppEvent);

    @Delete("delete from PP_EVENT where id = #{id}")
    int delete(long id);
    ...
    @Select("SELECT COUNT(1) FROM PP_EVENT WHERE id > 0")
    int count();
